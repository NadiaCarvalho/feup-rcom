...
int is_I_frame_valid(char *frame, int frame_len) {
    ...
    return frame[0] == FLAG && frame[1] == SEND && frame[3] == (frame[1] ^ frame[2]);
}
...
/**
* Read a frame from the serial port and check its validity and make sure it is not a duplicate
...
int ll_read(int fd, char *packet, int *len) {
    ...
    while (!read_succesful) {
        ...
        if (!is_I_frame_valid(frame, frame_len)) { //Invalid frame
            ...
            *len = frame_len - 6;
            ...
            if(bcc2 == frame[frame_len - 2]){ // valid BCC2 - may still be a duplicate

                reply = create_US_frame(&reply_len, RR);

                if(has_valid_sequence_number(frame[2])){ // new valid frame -> accept
                read_succesful = 1;
                }
                ...
                else reply = create_US_frame(&reply_len, RR); // duplicate frame, send RR and discard
            }
    ...
    int is_frame_DISC(char *reply) {
        ...
        int is_frame_RR(char *reply) {
            ...
            int has_valid_sequence_number(char control_byte){
                ...
                if (control byte & s << 6){
                    printf("correct seq num: %d\n", control byte & s << 6)
                }
                return (control byte & s << 6)
            }
