...
int set_up_connection(char *terminal, struct termios *oldtio, status stat){
  ...
  if(set_terminal_attributes(oldtio, &newtio) != 0){
    printf("application_layer :: set_up_connection() :: error setting terminal attributes\n");
    return -1;
  }
...
int set_terminal_attributes(struct termios *old_port_settings, struct termios *new_port_settings){
  ...
  if (tcgetattr(application.file_descriptor, old_port_settings) == -1) { /* save current port settings */
  ...
  if (tcsetattr(application.file_descriptor, TCSANOW, new_port_settings) == -1) {
